...
export function textoMemorialAcessoViatura(state, sistemas) {
  ...
  paragrafos.push(
    `A via de acesso possui largura de ${fmt(r.largura.adotada)} m e altura livre de ${fmt(r.alturaLivre.adotada)} m, ${ok(r.largura.atende && r.alturaLivre.atende)} às dimensões mínimas de ${fmt(r.largura.minima)} m e ${fmt(r.alturaLivre.minima)} m exigidas pelos itens 5.1.1.1 e 5.1.1.2 da NT 01/2024 CBMMA.`
  )

  paragrafos.push(
    r.carga.confirmado
      ? `O pavimento da via de acesso foi dimensionado para suportar viaturas com peso de ${r.carga.minimaKg / 1000} t distribuídas em ${r.carga.eixos} eixos, conforme item 5.1.1.3 da NT 01/2024 CBMMA.`
  ...
  paragrafos.push(
    `O desnível da via de acesso é de ${fmt(r.desnivel.longAdotado)}% no sentido longitudinal e ${fmt(r.desnivel.transvAdotado)}% no sentido transversal, ${ok(r.desnivel.atendeLong && r.desnivel.atendeTransv)} ao limite máximo de ${r.desnivel.maximo}% estabelecido pelo item 5.1.1.4 da NT 01/2024 CBMMA.`
  )

  if (r.portao) {
    paragrafos.push(
      `O portão de acesso possui largura de ${fmt(r.portao.largura.adotada)} m e altura de ${fmt(r.portao.altura.adotada)} m, ${ok(r.portao.largura.atende && r.portao.altura.atende)} às dimensões mínimas de ${fmt(r.portao.largura.minima)} m e ${fmt(r.portao.altura.minima)} m exigidas pelo item 5.1.1.5 da NT 01/2024 CBMMA.`
    )
  }

  paragrafos.push(
    r.retorno.exigeRetorno
      ? `Por possuir extensão de ${fmt(r.retorno.extensaoVia)} m, superior a ${r.retorno.gatilho} m, a via de acesso conta com retorno do tipo ${r.retorno.tipoRetorno || '[a definir]'}, conforme item 5.1.1.6 da NT 01/2024 CBMMA.`
      : `A via de acesso possui extensão de ${fmt(r.retorno.extensaoVia)} m, não ultrapassando o limite de ${r.retorno.gatilho} m que exigiria a previsão de retorno (item 5.1.1.6 da NT 01/2024 CBMMA).`
  )

  if (r.retorno.saidaIndependente) {
    paragrafos.push(
      `Como as ruas internas não possibilitam manobra de retorno da viatura para saída pelo portão de acesso, foi prevista saída independente com largura de ${fmt(r.retorno.saidaIndependente.largura.adotada)} m e altura de ${fmt(r.retorno.saidaIndependente.altura.adotada)} m, ${ok(r.retorno.saidaIndependente.largura.atende && r.retorno.saidaIndependente.altura.atende)} às dimensões mínimas exigidas pelo item 5.1.1.6.2 da NT 01/2024 CBMMA.`
    )
  }

  paragrafos.push(
    `A via de acesso dista ${fmt(r.distancia.adotada)} m d${r.distancia.baseadoEm === 'hidrante' ? 'o hidrante de recalque' : 'a edificação'}, ${ok(r.distancia.atende)} ao limite máximo de ${r.distancia.maxima} m estabelecido pelo item 5.1.1.7 da NT 01/2024 CBMMA${r.distancia.baseadoEm === 'hidrante' ? ' (edificação com previsão de sistema de hidrantes/mangotinhos)' : ''}.`
  )
