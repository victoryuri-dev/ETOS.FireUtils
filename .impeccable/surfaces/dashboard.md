...
- Resumo técnico e Sistemas aplicados ficam lado a lado a partir de 1050 px, esticados à mesma altura — a pilha de cards não carrega painel próprio (borda/fundo), só o card ativo é a superfície visível. As métricas do card empilham na vertical (nome no topo, as três métricas abaixo) em qualquer largura, e a navegação anterior/próxima mora dentro do próprio card. A lista de sistemas usa duas colunas.
- A partir de 1050 px para baixo, Resumo técnico e Sistemas aplicados empilham em uma coluna cada um com sua altura natural.
