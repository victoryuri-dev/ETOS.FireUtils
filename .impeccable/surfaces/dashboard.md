...
- O heatmap de atividade fica ao lado de Situação do projeto, com largura e altura definidas pelo próprio conteúdo do grid (não estica pra acompanhar a altura da caixa vizinha). Não tem título nem legenda visíveis — a caixa carrega o nome acessível via `aria-label`. Mostra sempre uma janela fixa de 2 meses corridos até hoje, com quadrado de tamanho fixo e pequeno (11px) — os quadrados nunca esticam pra preencher espaço. O dia atual sempre cai na última coluna (mais à direita, igual ao GitHub) e recebe destaque branco sólido em vez de seguir a escala de vermelho, ficando localizável mesmo sem atividade registrada. Abaixo do grid, um texto pequeno mostra a última alteração real do projeto (mesmo `updatedAt` usado em ProjetosPage).
