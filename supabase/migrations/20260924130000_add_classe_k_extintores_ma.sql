-- Adiciona a Classe K (óleos/gorduras de cozinha) ao catálogo de extintores
-- portáteis — não coberta pelo texto-base da NT 21 CBMMA transcrito na seed
-- original (20260906120003_seed_normas_extintores_ma.sql), então entra como
-- upsert incremental em vez de editar a migração já aplicada. As duas cargas
-- (1-A:K e 2-A:K, ambas em cilindro de 6 L) são as comercialmente
-- disponíveis e correspondem às famílias do plugin Revit (ver
-- "Fire Utils.tab/lib/family_library/Extintor de Incêndio/Extintor Portátil
-- - K.rfa"). Upsert substitui o `dados` inteiro — reenvia o payload completo
-- da seed anterior, só com os dois itens de Classe K a mais em
-- tipos_portatil.
...
insert into public.normas_dados (uf, sistema, dados, versao) values
  ...
  "tipos_portatil": [
    ...
    {
      "key": "k_1a",
      "label": "Classe K — 1-A:K",
      "capacidadeMinima": "1-A:K - 6L",
      "classes": [
        "A",
        "K"
      ]
    ...
    {
      "key": "k_2a",
      "label": "Classe K — 2-A:K",
      "capacidadeMinima": "2-A:K - 6L",
      "classes": [
        "A",
        "K"
      ]
